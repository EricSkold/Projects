...
def visualize(graph, view="dot", name="mygraph", nodecolors=None):
    dot = graphviz.Graph(engine=view, format="png", name=name)
    for v in graph.vertices():
        if v in nodecolors:
            dot.node(v,style="filled",fillcolor=nodecolors[v],label=v,)
        else:
            dot.node(v, label=v)
...
def view_shortest(G, source, target, cost=lambda u, v: 1):
    path = dijkstra(G, source, cost)[str(target)]["path"]
    print(path)
    colormap = {v: "#90EE90" for v in path}
    path_name = "from " + str(source) + " to " + str(target)
