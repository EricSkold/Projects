...
ORIGINAL_SVG = 'lab3/tram/templates/tram/images/gbg_tramnet.svg'
MY_GBG_SVG = 'my_gbg_tramnet.svg'  # the output SVG file
MY_TRAMNETWORK_JSON = 'lab3/static/tramnetwork.json'  # JSON file from lab1
TRAM_URL_FILE = 'stop_url_dictionary.json'  # given in lab3/files, replace with your own in bonus 2
...
def scaled_position(network):
    ...
    scalefactor = len(network)*1.5  # heuristic
...
# Here we modify the original svg map
def modify_svg():
    with open(ORIGINAL_SVG) as file:
        data = BeautifulSoup(file.read(), 'html.parser')
        stops = data.select('a')
        for stop in stops:
            stop.attrs['xlink:href'] = stop_url(stop['xlink:title'])
    with open(ORIGINAL_SVG, 'w') as new_file:
        new_file.write(data.prettify())
